    ...
    <div className="min-h-screen bg-white pb-24">
      ...
      <div className="px-6 pt-14 pb-4 bg-white/95 backdrop-blur-sm sticky top-0 z-20 border-b border-transparent">
        <div className="flex justify-between items-center mb-1">
          <h1 className="text-xl font-bold tracking-tight text-neutral-900">Chat de Apoyo</h1>
      ...
      <div className="flex-1 overflow-y-auto pb-6 bg-white">
        ...
        <div className="px-6 py-4" style={{ animation: 'slideUp 0.5s ease-out both' }}>
          <div className="relative w-full rounded-3xl overflow-hidden bg-neutral-900 p-6 shadow-xl text-center flex flex-col items-center">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <p className="text-sm text-neutral-300 mb-6 leading-relaxed max-w-[240px]">
                Estoy aquí para escucharte y ayudarte a procesar tus emociones. Sin juicios.
              </p>
              
              <button 
                onClick={handleStartChat}
                className="w-full py-3.5 bg-white text-indigo-950 rounded-xl font-semibold text-sm hover:bg-indigo-50 transition-all shadow-[0_0_15px_rgba(255,255,255,0.15)] flex items-center justify-center gap-2 transform active:scale-[0.98]"
              >
              ...
              <div className="mt-4 pt-4 border-t border-white/10 w-full flex items-center justify-center gap-2">
                ...
                <button 
                  ...
                  className="px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-white text-[10px] font-medium hover:bg-white/10 transition-colors flex items-center gap-1.5"
                >
        ...
        <div className="mt-2 mb-6" style={{ animation: 'slideUp 0.5s ease-out 0.1s both' }}>
          ...
          <div className="flex overflow-x-auto no-scrollbar px-6 gap-3 pb-2">
            {wellnessActions.map((action, index) => (
              <button
                key={index}
                onClick={() => handleWellnessAction(action.title)}
                className={`flex-shrink-0 w-32 p-3 rounded-2xl ${action.bgColor} border ${action.borderColor} flex flex-col gap-2 hover:${action.bgColor.replace('50', '100')} transition-colors text-left group`}
              >
        ...
        {/* Chat History */}
        <div className="px-6 pb-6" style={{ animation: 'slideUp 0.5s ease-out 0.2s both' }}>
          <h3 className="text-sm font-bold text-neutral-900 mb-3">Historial Reciente</h3>
          
          {isLoading ? (
            <div className="flex flex-col gap-2">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-neutral-100 animate-pulse">
